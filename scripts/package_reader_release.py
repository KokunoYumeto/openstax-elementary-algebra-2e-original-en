...
ROOT=Path(__file__).resolve().parents[1]; READER=ROOT/"output/html-en"; RELEASE=ROOT/"release/v1.0.1"
NAME="openstax-elementary-algebra-2e-original-en-html-v1.0.1.zip"; PREFIX="openstax-elementary-algebra-2e-original-en-html/"
    ...
    if not target.exists():
        with zipfile.ZipFile(target,"x",zipfile.ZIP_DEFLATED,compresslevel=9) as z:
            for name,path in sorted(actual.items()):
                info=zipfile.ZipInfo(PREFIX+name,(2026,9,6,0,0,0));info.compress_type=zipfile.ZIP_DEFLATED;info.external_attr=0o100644<<16
